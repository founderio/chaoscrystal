...
public class Config {
	...
	public static boolean glassesSneakToShowAspects = true;
	public static boolean glassesDebugMode = false;
	...
	public static int crystalAspectStorage = 1000000;
	public static int crystalHitsPerTick = 1;
	public static int crystalMaxTriesPerTick = 80;
	public static int crystalRange = 10;
	public static int crystalTickInterval = 1;
	public static int focusRange = 20;
	public static int focusTickInterval = 60;
	...
	private static synchronized void loadConfig() {
		...
		glassesSneakToShowAspects = config.getBoolean("sneak_to_show_aspects", Configuration.CATEGORY_GENERAL, true, "Require sneaking to display aspects in the crystal glasses.");
		glassesDebugMode = config.getBoolean("debug_glasses", Configuration.CATEGORY_GENERAL, false, "Enable the debug mode on crystal glasses. (Displays additional info, mostly useless for regular gameplay)");

		crystalAspectStorage = config.getInt("crystal_aspect", Configuration.CATEGORY_GENERAL, 1000000, 10 , 1000000000, "Configure the maximum capacity of a chaos crystal. This is the maximum amount of ALL aspects together.");

		crystalHitsPerTick = config.getInt("hits_per_tick", Configuration.CATEGORY_GENERAL, 1, 1, 265, "Maximum number of blocks or entities extracted per tick.");
		crystalMaxTriesPerTick = config.getInt("max_tries_per_tick", Configuration.CATEGORY_GENERAL, 80, 1, 256, "Maximum number of tries to find an extraction target per tick.");

		crystalRange = config.getInt("crystalrange",  Configuration.CATEGORY_GENERAL, 10, 1, 256, "Maximum range of the chaos crystal in blocks.");
		crystalTickInterval = config.getInt("crystal_tick_interval", Configuration.CATEGORY_GENERAL, 1, 1, 256, "Interval (in ticks) the chaos crystal updates. (That means direct extraction or infusion, not foci ticks.)");

		focusRange = config.getInt("focus_range", Configuration.CATEGORY_GENERAL, 20, 1, 265, "Maximum range of the foci in blocks.");
		focusTickInterval = config.getInt("focus_tick_interval", Configuration.CATEGORY_GENERAL, 60, 1, 265, "Interval (in ticks) the foci update. (That means e.g. crystal<>crystal or crystal<>machine transfer)");
